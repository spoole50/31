...
from game_logic import GameState, Player, AIDifficulty, create_new_game
...
class GameTable:
    ...
    def add_player(self, player_id: str, player_name: str, is_host: bool = False) -> bool:
        ...
        if player_id in self.players:
            return False  # Player already in table
    ...
    def start_game(self) -> bool:
        ...
        try:
            ...
            game_player_ids = list(self.game_state.players.keys())
            
            # Map human players first
            for i, table_player in enumerate(human_players):
                if i < len(game_player_ids):
                    game_player_id = [pid for pid in game_player_ids if pid.startswith('player_')][i]
                    self.player_id_mapping[table_player.id] = game_player_id
            
            # Map AI players
            ai_game_ids = [pid for pid in game_player_ids if pid.startswith('ai_')]
            for i, table_player in enumerate(ai_players):
                if i < len(ai_game_ids):
                    self.player_id_mapping[table_player.id] = ai_game_ids[i]
